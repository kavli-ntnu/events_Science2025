...
function [r2_store, p_store] = trajectory_speed_vs_movement_seq

ds = dir('N:\benjamka\events\data\sequence\smat_full_1s');
clear names
for i = 3:length(ds)
    names{i-2} = ds(i).name;
end
...
for i = 1:length(traj_names)
    ...
    % remove intertrials
    to_remove = abs(animal_acc) < 0.002 ;
    traj_acc = traj_acc(~to_remove);
    animal_acc = animal_acc(~to_remove);
...
%% example
load(fullfile(ds(1).folder, 'smat_n_4.mat'))
