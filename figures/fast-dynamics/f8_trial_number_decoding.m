...
function [decodeError, decodeError_sh] = f8_trial_number_decoding
...
% inds = ~cellfun(@isempty, (cellfun(@strfind, names, repmat({'decodeError_heldOutTrials_trial'}, 1, length(names)), 'uniformoutput', false)));
names = names(inds);
...
load figp
fixPlot(1:2, {'Actual', 'Shuffle'}, '', 'Decoding error (# of trials)')
xlim([0.5, 2.5])
figp_skinny = figp;
