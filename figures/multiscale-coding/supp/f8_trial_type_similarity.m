...
function [dists_pool_match, dists_pool_mismatch] = f8_trial_type_similarity

ds = dir('N:\benjamka\events\data\figure-eight\trialTypeSimilarity');
clear names
for i = 3:length(ds)
    names{i-2} = ds(i).name;
end
...
load figp
fixPlot(1:2, {'Same', 'Different'}, '', 'Distance between trials')
xlim([0.5, 2.5])
ymax = max(abs(dists_pool_mismatch - dists_pool_match));
figp_skinny = figp;
