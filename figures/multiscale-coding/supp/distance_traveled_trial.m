...
function [dists_pool, dists_pool_neigh] = distance_traveled_trial

ds = dir('N:\benjamka\events\data\figure-eight\dists');
clear names
for i = 3:length(ds)
    names{i-2} = ds(i).name;
end
inds_endpt = ~cellfun(@isempty, (cellfun(@strfind, names, repmat({'trial_endpt'}, 1, length(names)), 'uniformoutput', false)));
inds_neigh = ~cellfun(@isempty, (cellfun(@strfind, names, repmat({'trial_neigh'}, 1, length(names)), 'uniformoutput', false)));
...
for i = 1:length(endpt_names)
    ...
    dists_pool = [dists_pool, nanmean(dists_all(:))];
end

dists_pool_neigh = [];
for i = 1:length(neigh_names)
    load(fullfile(ds(1).folder, neigh_names{i}))
    dists_pool_neigh = [dists_pool_neigh, nanmean(dists_all(:))];
end
...
load figp
fixPlot(1:2, {'Endpoint', 'Neighbor'}, '', 'Distance traveled within trial')
xlim([0.5, 2.5])
figp_skinny = figp;
